...
export class PromotionsigmentnewheaderComponent extends AbstractComponent {
  gametitle:string = 'Promotions & Segments'
  isShow: boolean = true;
  ...
  override ngOnInit(): void {
    if (this.gamename == 'Promotion Sigment') {
      //this.promotionsigmentnewgame = true;
    }
  ...
  getTableData(tab: string) {
    let apiname = '/promotionsigment/fetchpromotionsigment';
  ...
  openDialog(): void {
    const dialogRef = this.dialog.open(PromotionsigmentnewfoodforthoughtComponent, {
      data: { name: "Cesim" },
    });
