...
@Component({
  selector: 'app-pricingcasemarket',
  standalone:true,
  imports: [CommonModule, FormsModule,MatIconModule,NgApexchartsModule, NgxSimpleTextEditorModule],
  templateUrl: './pricingcasemarket.component.html',
  styleUrls: ['./pricingcasemarket.component.scss']
})
export class PricingcasemarketComponent extends AbstractComponent {
  defaultcase: string = "";
  res: any = [];
          ...
          if (data.status == "Success") {
            if (data.resultList != null) {
              for (let i = 0; i < data.resultList.length; i++) {
                ...
                this.outlooktextcontent[i] = outlookdata.replace(/\\/g, '');
  ...
  contentvaluechange() {
    ...
    this._api.updatecasemanagementdata(String(round), "normal", "no", 'pricinggamecm', body, {}, apiname, 'pricinggamecmactivestatus').subscribe(
      {
        next: (data: any) => {
          if (data.status == "Success") {
            if (data.resultList != null) {
