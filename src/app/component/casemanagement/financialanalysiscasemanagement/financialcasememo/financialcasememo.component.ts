...
@Component({
  selector: 'app-financialcasememo',
  standalone: true,
  imports: [CommonModule, FormsModule,MatIconModule,NgApexchartsModule,NgxSimpleTextEditorModule],
  templateUrl: './financialcasememo.component.html',
  styleUrls: ['./financialcasememo.component.scss']
})

export class FinancialcasememoComponent extends AbstractComponent {
  defaultcase: string = "";
  ...
  contentvaluechange() {
    ...
    this._api.updatecasemanagementdata(String(round), "normal", "no", 'financialanalysiscm', body, {}, apiname, 'financialanalysiscmactivestatus').subscribe(
      {
        next: (data: any) => {
          if (data.status == "Success") {
            if (data.resultList != null) {
