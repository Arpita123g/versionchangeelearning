...
@Component({
  selector: 'app-accountingarabiccasefoodforthought',
  standalone: true,
  imports: [CommonModule, MatDialogModule, NgApexchartsModule,MatIconModule],   
  templateUrl: './accountingarabiccasefoodforthought.component.html',
  styleUrls: ['./accountingarabiccasefoodforthought.component.scss']
})
export class AccountingarabiccasefoodforthoughtComponent extends AbstractComponent {
  res: any = [];
  defaultcase: string = "";
  ...
  writeFoodForThoughtValue(cellname: string, index: number, value: any) {
    ...
    this._api.updatecasemanagementdata(String(index + 1), "foodforthought", "no", 'accountingarabiccm', body, {}, apiname, 'accountingarabiccmactivestatus').subscribe(
      {
        next: (data: any) => {
          if (data.status == "Success") {
