...
@Component({
  selector: 'app-businessgeneral',
  standalone: true,
  imports: [CommonModule, MatDialogModule, NgApexchartsModule,MatIconModule],
  templateUrl: './businessgeneral.component.html',
  styleUrls: ['./businessgeneral.component.scss']
})
export class BusinessgeneralComponent extends AbstractComponent {
  defaultcase: string = "";
  res: any = [];
  ...
  writeModuleValue(index: number, updatedsearchtypevalue: string, modulestatusvalue: string, cellname: string, value1: any, value2: any) {
    ...
    this._api.updatecasemanagementdata(String(index + 1), updatedsearchtypevalue, modulestatusvalue, 'businessbasiccasemanagement', body1, body2, apiname, 'businessbasiccmactivestatus').subscribe(
      {
        next: (data: any) => {
          if (data.status == "Success") {
            if (modulestatusvalue == 'yes') {
              this.res.businessBasicCMActiveStatus[index][cellname] = value2;
            }


          } else {
            if (modulestatusvalue == 'yes') {
              this.res.businessBasicCMActiveStatus[index][cellname] = this.res.businessBasicCMActiveStatus[index][cellname];
            } else {
