...
@Component({
  selector: 'app-ecommerccasemarket',
  standalone: true,
  imports: [CommonModule, FormsModule,MatIconModule,NgApexchartsModule, NgxSimpleTextEditorModule],
  templateUrl: './ecommerccasemarket.component.html',
  styleUrls: ['./ecommerccasemarket.component.scss']
})
export class EcommerccasemarketComponent extends AbstractComponent {
...
Instructorelementdetailssub: Subscription;
          ...
          if (data.status == "Success") {
            if (data.resultList != null) {
              ...
              for (let i = 0; i < data.resultList.length; i++) {
                let outlookdata = data.resultList[i].ecommercegameperioddata.b5;
  ...
  contentvaluechange() {
    ...
    this._api.updatecasemanagementdata(String(round), "normal", "no", 'ecommercegamecm', body, {}, apiname, 'ecommercegamecmactivestatus').subscribe(
      {
        next: (data: any) => {
          if (data.status == "Success") {
            if (data.resultList != null) {
