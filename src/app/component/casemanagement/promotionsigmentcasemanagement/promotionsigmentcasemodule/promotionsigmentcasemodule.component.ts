...
export class PromotionsigmentcasemoduleComponent extends AbstractComponent {
  ...
  writeModuleValue(index: number, updatedsearchtypevalue: string, modulestatusvalue: string, cellname: string, value1: any, value2: any) {
    ...
    this._api.updatecasemanagementdata(String(index + 1), updatedsearchtypevalue, modulestatusvalue, 'promotionscm', body1, body2, apiname, 'promotionscmactivestatus').subscribe(
      {
        next: (data: any) => {
          if (data.status == "Success") {
            if (modulestatusvalue == 'yes') {
              this.res.promotionscmActiveStatus[index][cellname] = value2;
            }
          } else {
            if (modulestatusvalue == 'yes') {
              this.res.promotionscmActiveStatus[index][cellname] = this.res.promotionscmActiveStatus[index][cellname];
            } else {
