...
@Component({
  selector: 'app-hrpcasemarket',
  standalone: true,
  imports: [CommonModule, FormsModule,MatIconModule,NgApexchartsModule, NgxSimpleTextEditorModule],
  templateUrl: './hrpcasemarket.component.html',
  styleUrls: ['./hrpcasemarket.component.scss']
})
export class HrpcasemarketComponent extends AbstractComponent {
  defaultcase: string = "";
  ...
  contentvaluechange() {
    ...
    this._api.updatecasemanagementdata(String(round), "normal", "no", 'hrplanningcm', body, {}, apiname, 'hrplanningcmactivestatus').subscribe(
      {
        next: (data: any) => {
          if (data.status == "Success") {
            if (data.resultList != null) {
