...
@Component({
  selector: 'app-itcasemanagementmodule',
  standalone:true,
  imports: [CommonModule, FormsModule,MatIconModule,NgApexchartsModule],
  templateUrl: './itcasemanagementmodule.component.html',
  styleUrls: ['./itcasemanagementmodule.component.scss']
})
export class ItcasemanagementmoduleComponent extends AbstractComponent {
  defaultcase: string = "";
  res: any = [];
  resoptional: any = [];
  ...
  writeModuleValue(index: number, updatedsearchtypevalue: string, modulestatusvalue: string, cellname: string, value1: any, value2: any) {
    ...
    this._api.updatecasemanagementdata(String(index + 1), updatedsearchtypevalue, modulestatusvalue, 'itmanagementcm', body1, body2, apiname, 'itmanagementcmactivestatus').subscribe(
      {
        next: (data: any) => {
          if (data.status == "Success") {
            if (modulestatusvalue == 'yes') {
              this.res.itManagementCMActiveStatus[index][cellname] = value2;
            }


          } else {
            if (modulestatusvalue == 'yes') {
              this.res.itManagementCMActiveStatus[index][cellname] = this.res.itManagementCMActiveStatus[index][cellname];
            } else {
