...
@Component({
  selector: 'app-capitalbudgetingcasemodule',
  standalone: true,
  imports: [CommonModule, FormsModule,MatIconModule,NgApexchartsModule],
  templateUrl: './capitalbudgetingcasemodule.component.html',
  styleUrls: ['./capitalbudgetingcasemodule.component.scss']
})
export class CapitalbudgetingcasemoduleComponent extends AbstractComponent {
  defaultcase: string = "";
  res: any = [];
  resoptional: any = [];
  ...
  writeModuleValue(index: number, updatedsearchtypevalue: string, modulestatusvalue: string, cellname: string, value1: any, value2: any) {
    ...
    this._api.updatecasemanagementdata(String(index + 1), updatedsearchtypevalue, modulestatusvalue, 'cbgamecm', body1, body2, apiname, 'cbgamecmactivestatus').subscribe(
      {
        next: (data: any) => {
          if (data.status == "Success") {
            if (modulestatusvalue == 'yes') {
              this.res.cbgamecmActiveStatus[index][cellname] = value2;
            }


          } else {
            if (modulestatusvalue == 'yes') {
              this.res.cbgamecmActiveStatus[index][cellname] = this.res.cbgamecmActiveStatus[index][cellname];
            } else {
