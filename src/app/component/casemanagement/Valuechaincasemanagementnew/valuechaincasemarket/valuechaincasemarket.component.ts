...
@Component({
  selector: 'app-valuechaincasemarket',
  standalone: true,
  imports: [CommonModule, FormsModule,MatIconModule,NgApexchartsModule, NgxSimpleTextEditorModule],
  templateUrl: './valuechaincasemarket.component.html',
  styleUrls: ['./valuechaincasemarket.component.scss']
})
export class ValuechaincasemarketComponent extends AbstractComponent {
  defaultcase: string = "";
  ...
  contentvaluechange() {
   ...
   let apiname = '/valuechaincm/updatevaluechaincm'
    ...
    this._api.updatecasemanagementdata(String(round), "normal", "no", 'valuechaincm', body, {}, apiname, 'valuechaincmactivestatus').subscribe(
      {
        next: (data: any) => {
          if (data.status == "Success") {
            if (data.resultList != null) {
