...
export class ConsumermoduleComponent extends AbstractComponent {
  ...
  writeModuleValue(index: number, updatedsearchtypevalue: string, modulestatusvalue: string, cellname: string, value1: any, value2: any) {
    ...
    this._api.updatecasemanagementdata(String(index + 1), updatedsearchtypevalue, modulestatusvalue, 'consumerbehaviourcm',body1, body2,apiname, 'consumerbehaviourcmactivestatus').subscribe(
      {
        next: (data: any) => {
          if (data.status == "Success") {
            if (modulestatusvalue == 'yes') {
              this.res.consumerBehaviourCMActiveStatus[index][cellname] = value2;
            } 
            

          } else {
            if (modulestatusvalue == 'yes') {
              this.res.consumerBehaviourCMActiveStatus[index][cellname] = this.res.consumerBehaviourCMActiveStatus[index][cellname];
            } else {
