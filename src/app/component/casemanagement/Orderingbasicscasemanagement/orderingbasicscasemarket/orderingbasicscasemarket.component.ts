...
@Component({
  selector: 'app-orderingbasicscasemarket', 
  standalone:true,
  imports: [CommonModule, FormsModule,MatIconModule,NgApexchartsModule, NgxSimpleTextEditorModule],
  templateUrl: './orderingbasicscasemarket.component.html',
  styleUrls: ['./orderingbasicscasemarket.component.scss']
})
export class OrderingbasicscasemarketComponent extends AbstractComponent {
  defaultcase: string = "";
  ...
  contentvaluechange() {
    ...
    this._api.updatecasemanagementdata(String(round), "normal", "no", 'orderingbasicscm', body, {}, apiname, 'orderingbasicscmactivestatus').subscribe(
      {
        next: (data: any) => {
          if (data.status == "Success") {
            if (data.resultList != null) {
