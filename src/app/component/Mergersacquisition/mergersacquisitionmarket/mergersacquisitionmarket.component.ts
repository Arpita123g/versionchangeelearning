...
@Component({
  selector: 'app-mergersacquisitionmarket',
  standalone:true,
  imports: [CommonModule, FormsModule,MatIconModule,NgApexchartsModule],
  templateUrl: './mergersacquisitionmarket.component.html',
  styleUrls: ['./mergersacquisitionmarket.component.scss']
})
export class MergersacquisitionmarketComponent extends AbstractComponent {
          ...
          if (data.status == "Success") {
            if (data.resultList != null) {
              ...
              for (let i = 0; i < this.periodcellvalue.length; i++) {
                this.result[i] = data.resultList[0].mergersAcquisitionCM[this.periodcellvalue[i]];
              }
              let outlookdata = data.resultList[0].mergersAcquisitionCM.b5;
