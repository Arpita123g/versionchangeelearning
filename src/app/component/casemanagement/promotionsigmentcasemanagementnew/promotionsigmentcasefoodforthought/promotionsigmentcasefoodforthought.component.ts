...
export class PromotionsigmentcasefoodforthoughtComponent extends AbstractComponent implements OnChanges, OnDestroy {
  res: any = [];
  defaultcase: string = "";
          ...
          if (data.status == "Success") {
            if (data.resultList != null) {
              ...
              for (let i = 0; i < this.foodforthought.length; i++) {
                ...
                //   this.foodforthought[i].option[0][j] = data.resultList[0].promotionsnewperioddata[this.foodforthought[i].option[0][j]];
                // }
                // for (let j = 0; j < this.foodforthought[i].feedback[0].length; j++) {
                //   this.foodforthought[i].feedback[0][j] = data.resultList[0].promotionsnewperioddata[this.foodforthought[i].feedback[0][j]];
                // }
                for (let j = 0; j < this.foodforthought[i].score[0].length; j++) {
                  this.foodforthought[i].score[0][j] = data.resultList[0].promotionsnewperioddata[this.foodforthought[i].score[0][j]];
                }
                for (let j = 0; j < this.foodforthought[i].rigor[0].length; j++) {
                  this.foodforthought[i].rigor[0][j] = data.resultList[0].promotionsnewperioddata[this.foodforthought[i].rigor[0][j]];
                }
                for (let j = 0; j < this.foodforthought[i].structuring[0].length; j++) {
                  this.foodforthought[i].structuring[0][j] = data.resultList[0].promotionsnewperioddata[this.foodforthought[i].structuring[0][j]];
                }
                for (let j = 0; j < this.foodforthought[i].synthesis[0].length; j++) {
                  this.foodforthought[i].synthesis[0][j] = data.resultList[0].promotionsnewperioddata[this.foodforthought[i].synthesis[0][j]];
                }
                for (let j = 0; j < this.foodforthought[i].business[0].length; j++) {
                  this.foodforthought[i].business[0][j] = data.resultList[0].promotionsnewperioddata[this.foodforthought[i].business[0][j]];
                }
              }
  ...
  writeFoodForThoughtValue(cellname: string, index: number, value: any) {
    ...
    this._api.updatecasemanagementdata(String(index + 1), "foodforthought", "no", 'promotionsnewcm', body, {}, apiname, 'promotionsnewcmactivestatus').subscribe(
      {
        next: (data: any) => {
          if (data.status == "Success") {
