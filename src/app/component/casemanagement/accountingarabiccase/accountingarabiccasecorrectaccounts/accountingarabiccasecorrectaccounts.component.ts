...
@Component({
  selector: 'app-accountingarabiccasecorrectaccounts',
  standalone: true,
  imports: [CommonModule, MatDialogModule, NgApexchartsModule,MatIconModule],
  templateUrl: './accountingarabiccasecorrectaccounts.component.html',
  styleUrls: ['./accountingarabiccasecorrectaccounts.component.scss']
})
export class AccountingarabiccasecorrectaccountsComponent extends AbstractComponent {
  defaultcase: string = "";
  res: any = [];
  ...
  writeMarketingValue(cellname: string, index: number, value: any) {
    ...
    this._api.updatecasemanagementdata(String(index + 1), "normal", "no", 'accountingarabiccm', body, {}, apiname, 'accountingarabiccmactivestatus').subscribe(
      {
        next: (data: any) => {
          if (data.status == "Success") {
