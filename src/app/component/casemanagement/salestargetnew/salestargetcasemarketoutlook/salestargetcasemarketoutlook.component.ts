...
@Component({
  selector: 'app-salestargetcasemarketoutlook',
  standalone: true,
  imports: [CommonModule, FormsModule, NgxSimpleTextEditorModule],
  templateUrl: './salestargetcasemarketoutlook.component.html',
  styleUrls: ['./salestargetcasemarketoutlook.component.scss']
})
export class SalestargetcasemarketoutlookComponent extends AbstractComponent {
  defaultcase: string = "";
  ...
  contentvaluechange() {
    ...
    this._api.updatecasemanagementdata(String(round), "normal", "no", 'salestargetcm', body, {}, apiname, 'salestargetcmactivestatus').subscribe(
      {
        next: (data: any) => {
          if (data.status == "Success") {
            if (data.resultList != null) {
