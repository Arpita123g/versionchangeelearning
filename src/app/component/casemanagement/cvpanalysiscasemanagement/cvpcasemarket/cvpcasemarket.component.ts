...
@Component({
  selector: 'app-cvpcasemarket',
  standalone: true,
  imports: [CommonModule, FormsModule,MatIconModule,NgApexchartsModule,NgxSimpleTextEditorModule],
  templateUrl: './cvpcasemarket.component.html',
  styleUrls: ['./cvpcasemarket.component.scss']
})
export class CvpcasemarketComponent extends AbstractComponent {
  ...
  contentvaluechange() {
    ...
    this._api.updatecasemanagementdata(String(round), "normal", "no", 'cvpanalysiscm', body, {}, apiname, 'cvpanalysiscmactivestatus').subscribe(
      {
        next: (data: any) => {
          if (data.status == "Success") {
            if (data.resultList != null) {
